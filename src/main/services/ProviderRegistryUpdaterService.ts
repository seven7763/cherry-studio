...
// defence, fallback-to-bundled the third.
const REMOTE_BRANCH = 'registry-data'
...
export class ProviderRegistryUpdaterService extends BaseService {
  ...
  private async fetchAndValidate(): Promise<{ staged: StagedFile[]; manifestBody: string } | null> {
    ...
    if (manifestBody === null || !this.passesReleaseFloor(manifestBody)) return null
  ...
  private passesReleaseFloor(manifestBody: string): boolean {
    let floor: unknown
    try {
      floor = JSON.parse(manifestBody).releaseFloor
    } catch {
      floor = undefined
    }
    const cleanFloor = typeof floor === 'string' ? semver.valid(semver.coerce(floor)) : null
    if (!cleanFloor) {
