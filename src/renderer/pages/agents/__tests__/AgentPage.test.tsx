    ...
    open ? (
      <div data-testid="agent-create-dialog">
        <button
          ...
          onClick={() => {
            onOpenChange?.(false)
            void onCreated?.(agentPageMocks.createdAgentIds.shift() ?? 'agent-b')
          }}>
...
describe('AgentPage', () => {
  beforeEach(() => {
    ...
    agentPageMocks.createdAgentIds = ['agent-b']
  ...
  it('creates and activates an empty session after creating an agent from the modern add entry', async () => {
    agentPageMocks.sessionDisplayMode = 'time'
  ...
  it('prevents duplicate empty session creation from rapid classic-layout agent creation callbacks', async () => {
    agentPageMocks.sessionDisplayMode = 'agent'
    agentPageMocks.sessionPanePosition = 'left'
    agentPageMocks.routeSearch = { sessionId: 'session-created' }
    agentPageMocks.agents = [
  ...
  it('queues distinct agent creation callbacks while another empty session is being created', async () => {
    agentPageMocks.sessionDisplayMode = 'agent'
    agentPageMocks.sessionPanePosition = 'left'
    agentPageMocks.routeSearch = { sessionId: 'session-created' }
    agentPageMocks.agents = [
      { id: 'agent-a', model: 'model-a', name: 'Agent A' },
      { id: 'agent-b', model: 'model-b', name: 'Agent B' },
      { id: 'agent-c', model: 'model-c', name: 'Agent C' }
    ]
    agentPageMocks.createdAgentIds = ['agent-b', 'agent-c']
    activeSessionMocks.session = { ...agentPageMocks.persistedSession, agentId: 'agent-a' }
    activeSessionMocks.sessionSource = 'query'
    let resolveFirstSession!: (session: unknown) => void
    agentPageMocks.dataApiPost
      .mockReturnValueOnce(
        new Promise<unknown>((resolve) => {
          resolveFirstSession = resolve
        })
      )
      .mockResolvedValueOnce({ ...agentPageMocks.persistedSession, id: 'session-c', agentId: 'agent-c' })
    ...
    await act(async () => {
      resolveFirstSession({ ...agentPageMocks.persistedSession, id: 'session-b', agentId: 'agent-b' })
      await Promise.resolve()
    })

    await waitFor(() => expect(agentPageMocks.dataApiPost).toHaveBeenCalledTimes(2))
    expect(agentPageMocks.dataApiPost).toHaveBeenLastCalledWith('/agent-sessions', {
      body: {
        agentId: 'agent-c',
        name: '',
        workspace: { type: AGENT_WORKSPACE_TYPE.SYSTEM }
  ...
  it('uses the remembered workspace when creating an empty session from the classic-layout add entry', async () => {
    agentPageMocks.sessionDisplayMode = 'agent'
    agentPageMocks.routeSearch = { sessionId: 'session-existing' }
  ...
  it('reuses the agent latest empty session instead of creating another one from the classic-layout add entry', async () => {
    agentPageMocks.sessionDisplayMode = 'agent'
    agentPageMocks.routeSearch = {}
    agentPageMocks.agents = [
      { id: 'agent-a', model: 'model-a', name: 'Agent A' },
  ...
  it('reuses the latest empty system session and deletes duplicate empty system sessions from the classic-layout add entry', async () => {
    agentPageMocks.sessionDisplayMode = 'agent'
    agentPageMocks.routeSearch = {}
    agentPageMocks.agents = [
      { id: 'agent-a', model: 'model-a', name: 'Agent A' },
  ...
  it('creates a new session when the agent latest session is not empty from the classic-layout add entry', async () => {
    agentPageMocks.sessionDisplayMode = 'agent'
    agentPageMocks.routeSearch = {}
    agentPageMocks.agents = [
      { id: 'agent-a', model: 'model-a', name: 'Agent A' },
