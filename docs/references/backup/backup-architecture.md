...
> **TL;DR**: Break apart backup's centralized rule stores (`DomainRegistry`/`DomainStripper`/`DomainImporter`/`FileCollector`) into declarative `BackupContributor`s per business domain, and introduce **aggregate boundaries (AggregateBoundary)** so that conflict strategies propagate along whole-object boundaries in a statically verifiable way. Restore runs a full-database DB pre-snapshot (`VACUUM INTO`) + file snapshot + layered `withWriteTx` execution (file IO outside the transaction, DB import inside the transaction, aligning with the V2 `withWriteTx` philosophy).
...
When reviewing any mechanism, first ask "which requirement above does it serve". For example: "skip by whole object" serves main scenarios ①②; "pre-restore snapshot" serves restore safety and local undo (main scenario ③, loss prevention); while "ID remap" has no matching requirement (already excluded by the data prerequisites) → judged redundant and removed.
...
flowchart LR
  subgraph S1[Backup generation]
    ...
    A3 --> A4[VACUUM INTO copy as backup copy]
  ...
  subgraph S3[Restore consumption]
    ...
    C3 --> C4[Import rows defer FK via withWriteTx]
...
|---|---|---|
| Entity facts (schema) | Table ownership, reference facts, primary key shape, aggregate boundary, file-ref source, JSON soft reference | SET_NULL/DELETE_ROW actions, import order, restore strategy |
...
| Operations | File resource discovery, beforeArchive, per-row transform, afterImport, blob restore, cloneAggregate | Facts and policies expressible as pure data |
...
Four test categories: tsc + codegen check, coverage, equivalence, restore tests (aggregate conflict + pre-snapshot rollback + identity propagation: DB owning FK and required JSON ref → natural-key target; polymorphic soft-ref pin/entity_tag selected-domain filter and RENAME case).
...
| AGENTS | agent_workspace/agent_channel single-table renamable:false; agent_channel_task is a junction (double cascade FK); **job_schedule.type='agent.task' row-scope assigned to AGENTS** (natural-key `(type,name)`, FIELD_MERGE; Agent task definition, otherwise user tasks are lost by design); when job_schedule is merged by `(type,name)`, `agent_channel_task.taskId` (→ schedule id) must be rewritten via identity propagation to the local canonical schedule id (§5.4); afterImport must re-arm job_schedule timer (DB import does not call registerJobSchedule, otherwise agent.task does not fire until restart) |
...
**Implementation caveat**: ① post-restore consistency check (no dangling ref / no file_entry missing blob, rollback on failure); ② file_entry soft delete vs ref hard delete asymmetry (export filter must only take `deletedAt IS NULL`).
...
`BackupContributorPolicy`: `omittedReferenceOverrides` (exceptions only, must bind to fact + non-redundant + reason), `uniqueMergeRules`, `fieldMergePolicies` (FIELD_MERGE field-level merge), **`platformSpecificKeys?`** (PREFERENCES only: declare platform-related key patterns, e.g. `shortcut.*` / `*.path` / `app.hardware_acceleration`, **excluded** at restore — not imported cross-platform, to avoid non-existent paths/wrong shortcuts; portable keys normally SKIP + backfill; specific list provided by PREFERENCES owner. **finalize validation**: key pattern valid (glob syntax) + only PREFERENCES domain may declare, non-PREFERENCES declaration rejected). **Does not include** restoreRemap / idStrategies (over-design, removed).
...
| 21 | natural-key/slot aggregate's explicit conflictDefault is not SKIP —— **settings-class exception** (preference/note: SKIP local-first + backfill + `platformSpecificKeys` excludes cross-platform incompatible key, reason: user settings local-first, platform-specific not cross-platform; distinct from PROVIDERS credential merge) | `{ domain, aggregate, identityClass, conflictDefault }` |
...
- Invariant 13 (unique-backing) requires codegen to generate `DB_UNIQUE_KEYS` (three sources: `getTableConfig()` indexes(unique)/uniqueConstraints/column isUnique) as data source
...
### 9. Pre-restore snapshot and undo restore (restore orchestration layer)

The current file-level rollback only covers FILE_STORAGE overwrite, not mid-flight failure of DB row import, nor SQLite data such as API key / preference / provider / assistant / agent / chat history. Add restore orchestration layer RestoreRecoveryPoint: full-database DB pre-snapshot + restore journal + affected file snapshot (same restoreId). **Strict execution layering** (aligns with V2 withWriteTx "DB ops only inside fn, no file IO" constraint):

0. **manifest version gate** (first step of restore, before RESTORE BARRIER; read-only/operate on backup files, **does not touch live DB**):
   ...
   - **Failure cleanup**: migrate-forward failure (backup corrupted / migration SQL execution failure) = gate failure; abort restore, retain live DB original state (untouched), delete temporary `backup.sqlite`, **does not trigger step 4 full-database rollback** (because live DB unchanged), and report error to user "backup cannot be restored: file corrupted or unsupported version"
...
1. After **RESTORE BARRIER** acquire (silence WhenReady DB writers + block renderer mutation, throughout), via `createSnapshot` create VACUUM INTO snapshot (outside transaction; better-sqlite3 single-connection synchronous, serialized by construction, no extra write lock needed)
2. contributor restoreResources file IO outside withWriteTx, before
3. DB row import only inside withWriteTx
4. Failure full-database rollback is an application-level action (SQLite single-connection holds file lock — better-sqlite3 too, cannot directly rename-replace file): checkpoint(TRUNCATE) then close connection, then safe file promotion (integrity_check + fsync+rename atomic replace live .sqlite + delete stale -wal/-shm + rename-aside fallback), finally reconnect

This plan makes RestoreRecoveryPoint (full-database DB pre-snapshot + restore journal + affected file snapshot) an in-scope must-deliver item (**current restore safety is entirely unimplemented**: no `createSnapshot` repo-wide, no RestoreSafetyManager, no rollback/undo/journal/file snapshot, restore failure has no recovery guarantee; this plan fills the gap via DbService `createSnapshot` + `restoreDbFromSnapshot` — snapshot + rollback + journal + file snapshot + failure blocking). **Snapshot creation failure SHALL block restore** (breaking change, no such gate currently). **Under merge semantics the primary value is "undo successful restore"** (user rollback), secondary to failure rollback. Contributor is not responsible for full-database snapshot and rollback.

**Owner separation** (recovery point lifecycle): `barrier` + `createRecoveryPoint`/`commit`/`rollback` belong to `RestoreSafetyManager` (RESTORE BARRIER owner must be neutral, does not write DB, to prevent "both referee and player" self-reference); `RestoreRecoveryPointStore` + GC + `BackupV2_*` IPC (including undo orchestration) belong to `BackupService`; `undoRestore` is a `BackupService` orchestration method (query store + delegate `RestoreSafetyManager.rollback`); on-boot crash recovery gate is independent in `DbService.onInit` (BeforeReady).

> [!IMPORTANT]
> Inside the restore write transaction, PRAGMA defer_foreign_keys=ON (not foreign_keys=OFF — the latter is explicitly a no-op inside a transaction per SQLite docs, and DbService replays foreign_keys=ON on each reconnect). `defer_foreign_keys` only defers FK constraint *enforcement* to COMMIT (`foreign_key_check` validates whole-graph consistency before COMMIT), and does **not** disable ON DELETE *actions* (CASCADE/SET NULL still trigger immediately) — so the importer must follow "OVERWRITE row-level full replace via **upsert (`ON CONFLICT(identityKey) DO UPDATE`)**, do not DELETE parent rows; explicit cascade (DELETE_ROW) only deletes **leaf/junction rows** (no ON DELETE child action beneath)", to avoid double trigger with SQLite ON DELETE. ReferenceKind must faithfully replicate schema onDelete (cascade/restrict → owning/junction, set null/no action → optional, set default → reject), validated by invariant 19. (Note: ON DELETE RESTRICT is not deferrable by defer_foreign_keys and always errors immediately, unlike NO ACTION; this architecture's importer only deletes leaf/junction, so RESTRICT does not trigger in practice.) DB writes go via DbService.withWriteTx (DB ops only inside fn, file restore already outside transaction).
>
> **Restore safety trio (addressing PR #12659 review B1/B2/B3)**: ① RESTORE BARRIER (application-level write barrier, distinct from per-transaction `withWriteTx` — better-sqlite3 single-connection synchronous, writeMutex removed with #16626, write serialization by construction) silences WhenReady DB writers + blocks renderer mutation, across snapshot-file-DB-promote throughout; ② safe file promotion rollback sequence prevents WAL sidecar replay overwriting snapshot; ③ journal persistent state machine (6 states) + on-boot crash recovery + completed gate; **recoverOnBoot as the gate before `migrateDb` inside `DbService.onInit`** (target state: insert recovery gate in `DbService.ts:139` onInit, making the order configurePragmas → **recovery gate** → migrateDb → seeders; current onInit only has the first three steps, no gate) — when the journal has non-terminal entries, first run compensating rollback (live DB full-database rollback via `DbService.restoreDbFromSnapshot`; overwritten files restored to pre-restore from `fileSnapshots`, symmetric with runtime rollback, not via RSM) then release migrateDb, to avoid migrateDb/seeders touching a half-recovered DB; rollback snapshot schema may be older than consumer, after release migrateDb reuses the step 0 migrate-forward mechanism to advance.
>
> **Upstream prerequisites (gating)**: depends on new DbService `createSnapshot` (build full-database snapshot outside transaction, dedicated VACUUM INTO; better-sqlite3 single-connection synchronous, serialized by construction, no extra write lock needed) + `restoreDbFromSnapshot` (**full-database rollback composite API**, including checkpoint/close/safe-promote/reconnect/validation; rollback + recoverOnBoot two entry points share, eliminate drift) + `verifyLiveDb` (completed gate) + PreferenceService.reloadFromDb + DataApiService.armMutationGate/disarmMutationGate + PreferenceService.armWriteGate/disarmWriteGate (RESTORE BARRIER gate); must merge upstream API PR first, then merge backup implementation.
>
> **Preference cache consistency** (M1): `PreferenceService` does a one-time load of DB into in-memory cache at startup and no longer re-reads; so PREFERENCES domain `afterImport` must trigger `PreferenceService.reloadFromDb()+rebroadcast` to invalidate and reload main + each renderer cache; after full-database rollback (live DB replaced) also trigger (all caches invalidated). Otherwise restored/rolled-back preferences silently ineffective on runtime until restart.
...
- Restore conflict default by identityClass: uuid-entity default SKIP (skip conflict, retain local version); natural-key/slot default FIELD_MERGE (retain local credential + merge remote, prevent data loss); **settings-class exception** (preference/note: SKIP local-first + backfill + `platformSpecificKeys` excludes cross-platform incompatible key, see §3/§6 policy). Conflict is judged by the user-comprehensible minimal whole object (aggregate root). Locally existing records are always retained (merge semantics). FIELD_MERGE via field-level merge **modifies in place** the local row fields existing on both sides (not only add not delete, but local override), so local row modifications on this path must be within the §9 pre-snapshot rollback protection scope (accidental modification undoable).
...
- Restore write path goes via `DbService.withWriteTx` + `defer_foreign_keys=ON` (not FK OFF) explicit cascade.
